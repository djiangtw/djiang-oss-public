/**
 * danieRTOS v1.x - Producer/Consumer Demo
 *
 ...
 *
 * NOTE: This demo runs tasks in M-mode because semaphore/queue syscalls
 * are not yet implemented. The tasks use kernel APIs directly.
 * Future versions will add syscall wrappers for IPC primitives.
 */
...
/* -----------------------------------------------------------------------------
 * Task Functions (run in M-mode for now - uses kernel APIs directly)
 * ---------------------------------------------------------------------------*/
...
void demo_init(void)
{
    ...
    /* Run in M-mode (TASK_PRIV_M) because sem/queue syscalls not implemented */
    task_create_priv("blinker",  task_blinker,  NULL, 1, stack1, sizeof(stack1), TASK_PRIV_M);
    task_create_priv("producer", task_producer, NULL, 2, stack2, sizeof(stack2), TASK_PRIV_M);
    task_create_priv("consumer", task_consumer, NULL, 3, stack3, sizeof(stack3), TASK_PRIV_M);

    uart_printf("Producer/Consumer Demo - 3 tasks (M-mode)\n\n");
}
