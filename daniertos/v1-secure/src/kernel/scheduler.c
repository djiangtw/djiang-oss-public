...
/* Delayed task list (sorted by wake_tick) */
static tcb_t *g_delayed_list = NULL;

void sched_add_delayed(tcb_t *task)
{
    /* Insert sorted by wake_tick */
    tcb_t **pp = &g_delayed_list;
    while (*pp != NULL && (*pp)->wake_tick <= task->wake_tick) {
        pp = &(*pp)->next;
    }
    task->next = *pp;
    *pp = task;
...
/**
 * Check delayed tasks and wake those whose time has come
 * Called from tick_increment
 */
void sched_check_delayed(void)
{
    tick_t now = tick_get();

    while (g_delayed_list != NULL && g_delayed_list->wake_tick <= now) {
        tcb_t *task = g_delayed_list;
        g_delayed_list = task->next;
        task->next = NULL;
